...
applyBigPayment payment (Loan p r mp tm ti) = (Loan (p-((mp+payment)-i)) r mp (tm+1) (ti+i))
...
payOffLoans extraCash (l:ls)  
  ...
  | otherwise = payOffLoans extraCash ((applyBigPayment extraCash l):map applyMinPayment ls) -- same as (l:ls)
